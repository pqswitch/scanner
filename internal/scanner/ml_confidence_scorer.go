...
func (ml *MLConfidenceScorer) extractTestFileFeature(context string, fileCtx *FileContext) float64 {
	...
	if fileCtx.IsTest {
		// Special handling for crypto detection test files - these are SUPPOSED to contain crypto
		if strings.Contains(strings.ToLower(fileCtx.FilePath), "crypto_rules") ||
			strings.Contains(strings.ToLower(fileCtx.FilePath), "crypto_test") ||
			strings.Contains(strings.ToLower(fileCtx.FilePath), "detection_test") {
			return 0.0 // No penalty for crypto detection test files
		}
		return 0.3 // Reduced penalty for regular test files
	}
